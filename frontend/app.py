...
Single Streamlit app (CSV input only)
"""
...
SEVERITY_COLOR = {"Low": "#a7f3d0", "Medium": "#fde68a",
                  "High": "#fca5a5", "Critical": "#f87171", "None": "#d1d5db"}
PATCH_HINTS = {
...
def apply_patch(df, sw):
    ...
    df.loc[i, "applied_patch_version"] = reco.split(">=")[-1].strip() if ">=" in str(reco) else "latest"
    return df, True
...
with st.expander("📥 Upload Inventory (CSV only)", expanded=True):
    ...
    if uploaded:
        ...
        # expected columns: software, version
